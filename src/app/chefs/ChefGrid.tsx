...
const ChefGrid = () => {
  return (
    <div className="p-6 mt-20 max-w-[1350px] mx-auto">
      {/* Grid with responsive columns */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-6">
        {chefs.map((chef, index) => (
          <div
            key={index}
            className={`relative overflow-hidden rounded-xl shadow-lg bg-white flex flex-col transition-transform transform hover:scale-105 hover:shadow-xl ${
              index === 6
                ? "border-4 border-yellow-500" // Complete purple border for the 7th box
                : "border-4 border-transparent hover:border-yellow-500" // Hover effect for other boxes
            }`}
          >
            {/* Chef Image */}
